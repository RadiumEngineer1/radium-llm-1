      ...
      <div className="flex gap-3">
        ...
        <div className="shrink-0">
          <svg width="70" height="70" viewBox="0 0 70 70" className="opacity-60">
            {/* Outer ring */}
            <circle cx="35" cy="35" r="32" fill="none" stroke={isAbom ? 'var(--hex-danger)' : 'var(--hex-accent)'} strokeWidth="0.5" opacity="0.3" />
            <circle cx="35" cy="35" r="22" fill="none" stroke={isAbom ? 'var(--hex-danger)' : 'var(--hex-accent)'} strokeWidth="0.5" opacity="0.2" />
            <circle cx="35" cy="35" r="12" fill="none" stroke={isAbom ? 'var(--hex-danger)' : 'var(--hex-accent)'} strokeWidth="0.5" opacity="0.15" />
            {/* Crosshair */}
            <line x1="35" y1="3" x2="35" y2="67" stroke={isAbom ? 'var(--hex-danger)' : 'var(--hex-accent)'} strokeWidth="0.3" opacity="0.15" />
            <line x1="3" y1="35" x2="67" y2="35" stroke={isAbom ? 'var(--hex-danger)' : 'var(--hex-accent)'} strokeWidth="0.3" opacity="0.15" />
            {/* Sweep beam */}
            <line x1="35" y1="35" x2="35" y2="3" stroke={isAbom ? 'var(--hex-danger)' : 'var(--hex-accent)'} strokeWidth="1" opacity="0.6">
              <animateTransform attributeName="transform" type="rotate" from="0 35 35" to="360 35 35" dur={isAbom ? '3s' : '4s'} repeatCount="indefinite" />
            </line>
            {/* Sweep fade trail */}
            <path d="M35,35 L35,3 A32,32 0 0,1 62,18 Z" fill={isAbom ? 'var(--hex-danger)' : 'var(--hex-accent)'} opacity="0.08">
              <animateTransform attributeName="transform" type="rotate" from="0 35 35" to="360 35 35" dur={isAbom ? '3s' : '4s'} repeatCount="indefinite" />
            </path>
            {/* Blips */}
            {status === 'online' && (
              <>
                <circle cx="28" cy="20" r="2" fill={isAbom ? 'var(--hex-danger)' : 'var(--hex-success)'} opacity="0.7">
                  <animate attributeName="opacity" values="0.7;0.2;0.7" dur="2s" repeatCount="indefinite" />
                </circle>
                <circle cx="45" cy="28" r="1.5" fill={isAbom ? 'var(--hex-danger)' : 'var(--hex-success)'} opacity="0.5">
                  <animate attributeName="opacity" values="0.5;0.1;0.5" dur="3s" repeatCount="indefinite" />
                </circle>
                {isGenerating && (
                  <circle cx="38" cy="42" r="2.5" fill={isAbom ? 'var(--hex-danger)' : 'var(--hex-accent2)'} opacity="0.8">
            ...
            {/* Center dot */}
            <circle cx="35" cy="35" r="1.5" fill={isAbom ? 'var(--hex-danger)' : 'var(--hex-accent)'} opacity="0.8" />
          </svg>
        </div>
