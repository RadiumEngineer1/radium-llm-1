/**
 * DnaHelix — Pure SVG double helix. CSS animation scrolls it horizontally.
 */
export default function DnaHelix() {
  const h = 30;
  const w = 500;  // wide enough for seamless loop (will scroll half)
  const steps = 40;
  const amplitude = 10;
  const wavelength = 250; // pixels per full wave
      ...
      >
        {rungs.map((r, i) => (
          <line key={i} x1={r.x} y1={r.y1} x2={r.x} y2={r.y2}
            stroke="var(--color-accent)" strokeWidth="0.8" opacity={r.op} />
        ))}
        <path d={toPath(points1)} fill="none" stroke="var(--color-accent)" strokeWidth="1.5" opacity="0.5" />
        <path d={toPath(points2)} fill="none" stroke="var(--color-accent2)" strokeWidth="1.5" opacity="0.5" />
        {points1.filter((_, i) => i % 4 === 0).map(([x, y], i) => (
          <circle key={`a${i}`} cx={x} cy={y} r="1.5" fill="var(--color-accent)" opacity="0.6" />
        ))}
        {points2.filter((_, i) => i % 4 === 2).map(([x, y], i) => (
          <circle key={`b${i}`} cx={x} cy={y} r="1.5" fill="var(--color-accent2)" opacity="0.6" />
        ))}
      </svg>
    </div>
  );
